...
public class Character {
	...
	public String toString() {
		return name + " is a member of" + allegiance + " and his message is" + messagesFile + ".";
	}
	...
	public static int howHappy(Character c) {
		...
		for (int i = 0; i < FileHelper.loadMessages(c.messagesFile).size(); i++) {
			...
			for (String word : words) {
				...
				for (String happyEmotions : Emotions.happy()) {
					if (word.contains(happyEmotions)) {
						happy++;
					} else {
						continue;
					}
	...
	public static int howSad(Character c) {
		...
		for (int i = 0; i < FileHelper.loadMessages(c.messagesFile).size(); i++) {
			...
			for (String word : words) {
				...
				for (String sadEmotions : Emotions.sad()) {
					if (word.contains(sadEmotions)) {
						sad++;
					} else {
						continue;
					}
	...
	public static int loving(Character c) {
		...
		for (int i = 0; i < FileHelper.loadMessages(c.messagesFile).size(); i++) {
			String[] words = FileHelper.loadMessages(c.messagesFile).get(i).split(" ");
			for (String word : words) {
				for (String lovingEmotions : Emotions.loving()) {
					if (word.contains(lovingEmotions)) {
						loving++;
					} else {
						continue;
					}
	...
	public static void main(String[] args) {
		...
		ArrayList<ArrayList<String>> md = MetaData.readFile();

		Character daenerysStormborn = new Character(md.get(0).get(0), md.get(1).get(0), md.get(2).get(0),
				new ImageIcon("D:\\JAVA 2\\Lesson9\\resource\\daenerys.png"));
		Character jonSnow = new Character(md.get(0).get(1), md.get(1).get(1), md.get(2).get(1),
				new ImageIcon("D:\\JAVA 2\\Lesson9\\resource\\jon.png"));
		Character tyrionLannister = new Character(md.get(0).get(2), md.get(1).get(2), md.get(2).get(2),
				new ImageIcon("D:\\JAVA 2\\Lesson9\\resource\\tyrion.png"));
		Character cerseiLannister = new Character(md.get(0).get(3), md.get(1).get(3), md.get(2).get(3),
				new ImageIcon("D:\\JAVA 2\\Lesson9\\resource\\cersei.png"));
		...
		daenerysStormborn.printMessages();
		System.out.println();
		...
		lovesMore(daenerysStormborn, jonSnow);
